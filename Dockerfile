...
COPY docker-debian-opencv2/jupyter_notebook_config.py /root/.jupyter/jupyter_notebook_config.py
COPY docker-debian-opencv2/run_jupyter /bin/run_jupyter
RUN chmod +x /bin/run_jupyter
EXPOSE 9999

# X11VNC configuration
RUN mkdir -p /root/.vnc && x11vnc -storepasswd 1234 /root/.vnc/passwd
COPY docker-debian-opencv2/run_vnc /bin/run_vnc
...
COPY docker-debian-opencv2/run_ssh /bin/run_ssh
RUN chmod +x /bin/run_ssh
EXPOSE 22

WORKDIR /usr/local/anpr
